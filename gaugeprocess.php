...
function getIPAddresses() {
    ...
    $data = shell_exec("sudo ifconfig | awk '/inet / {print $2}'");

    // Explode the output string by newline characters to get individual IP addresses
    $ipAddresses = explode("\n", trim($data));
...
function getPorts() {
    ...
    $data = shell_exec("sudo netstat -tuln | awk '/LISTEN/ {print $4}'");

    // Explode the output string by newline characters to get individual ports
    $entries = explode("\n", trim($data));

    $ports = [];
    // Iterate over each entry to extract the port number
    ...
    // Return the array of port numbers
    return $ports[1][1];
}
